...
from blankly.exchanges.interfaces.xtb.xtb_logger import XTBLogger
...
class APIClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, port=DEMO_XAPI_PORT, encrypt=True):
    ...
    def execute(self, dictionary):
        ...
        response = self._readObj()
        if "status" in response:
            if not response["status"]:
                raise ValueError(f"FAIL: {response}")
        return response
...
class APIStreamClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, port=DEMO_XAPI_STREAMING_PORT, encrypt=True, ssId=None,
    ...
    def _readStream(self):
        while (self._running):
            msg = self._readObj()
            if not msg["status"]:
                logger.error(f"Stream received: {msg}")
                continue
            logger.info(f"Stream received: {msg}")
            if (msg["command"] == 'tickPrices'):
                self._tickFun(msg["data"])
            elif (msg["command"] == 'trade'):
                self._tradeFun(msg["data"])
            elif (msg["command"] == "balance"):
                self._balanceFun(msg["data"])
            elif (msg["command"] == "tradeStatus"):
                self._tradeStatusFun(msg["data"])
            elif (msg["command"] == "profit"):
                self._profitFun(msg["data"])
            elif (msg["command"] == "news"):
                self._newsFun(msg["data"])
